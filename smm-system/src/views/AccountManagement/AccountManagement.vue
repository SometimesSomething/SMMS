...
export default {
  ...
  methods: {
    // getAccount(){
    //     this.request.get("http://127.0.0.1:5555/Account/AccountList")
    //         .then(response =>{
    ...
    handleEdit(id) {
      ...
      this.request
        .get(`/Account/AccountChange`, params)
        .then(response => {
    ...
    saveNewmsg(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.request
            .post("/Account/SaveeditAccount", params)
            .then(response => {
              let { error_code, reason } = response;
              if (error_code === 0) {
                this.$message({
    ...
    .el-message-box {
      ...
      .el-message-box__btns {
        .el-button--small {
          color: #fff;
          margin-right: 10px;
          background: rgba(236, 0, 13, 0.81);
          border: rgba(133, 0, 13, 0.86);
        }
